...
export function ChatInput() {
  ...
  return <div className="p-6 px-0 py-[14px]">
      <div className="text-center mb-4">
        ...
        <p className="text-sm font-light opacity-80" style={{ color: 'hsl(var(--chat-title-text))' }}>
          Converse com seu mentor particular e libere sua mente para o que realmente importa
        </p>
      </div>
      
      <form onSubmit={handleSubmit} className="relative max-w-2xl mx-auto">
        <div className="relative rounded-2xl backdrop-blur-lg border border-white/20 transition-all duration-300 hover:border-white/30 before:absolute before:inset-0 before:rounded-2xl before:bg-gradient-to-r before:from-white/5 before:to-transparent before:pointer-events-none bg-white/[0.49]" style={{ boxShadow: '0 25px 50px -12px hsl(var(--input-block-shadow) / 0.25)' }}>
          <input type="text" value={input} onChange={e => setInput(e.target.value)} placeholder="Escreva qualquer coisa..." className="w-full border-0 rounded-2xl text-[hsl(var(--input-text))] placeholder:text-[hsl(var(--input-placeholder))] focus:outline-none focus:ring-2 focus:ring-white/30 transition-all bg-transparent px-[20px] py-[25px] pr-16 relative z-10" />
